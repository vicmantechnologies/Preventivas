...
{
	$informacion=mysql_query("select idViaje from revisionesviaje where placaViaje = '".$_POST["placas_finalizadas"]."' order by idViaje desc limit 1");
	$row_informacion=mysql_fetch_array($informacion);
	?>
	<script>window.open('imp_viaje.php?idviaje=<?php echo $row_informacion["idViaje"]?>', 'name', 'location=yes,menubar=no,status=no,toolbar=no,scrollbars=yes,width=700,height=540')</script>
    <?php
	mysql_query("UPDATE revisiones set impreso=1 where idViaje = '".$row_informacion["idViaje"]."' ");
}
...
<form class="iform2" action="" method="post" name="registrar">
  ...
  <tr>
    ...
    <td width="30%" colspan="2"><span id="sprytextfield1">
      ...
      <input class="itext3" type="text" name="placas_finalizadas" id="placas_finalizadas" onKeyUp="this.value=this.value.toUpperCase()" autofocus>

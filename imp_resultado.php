...
{
	?>
	<script>window.open('imprimir.php?id_revision=<?php echo $_POST["placas_finalizadas"]?>', 'name', 'location=yes,menubar=no,status=no,toolbar=no,scrollbars=yes,width=700,height=540')</script>
    <?php
...
require("conexion.php");
	...
	//echo $result_ciudad_user["id_ciudad"];
        ...
        $query_convenios="Select id_rev,placa from revisiones WHERE finalizado=1 and impresion=0 and sede='".$result_ciudad_user["id_ciudad"]."' ORDER BY hora ASC ";
...
echo '<option value="'.$row_convenios["id_rev"].'">'.$row_convenios["placa"];}
echo '</select>';
?>
    </span></td>

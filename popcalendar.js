...
//---------------------------------------------TRADUCCION EN ESPAÑOL-----------------------------------------------------------------------------
	...
	function init()	{
		if (!ns4)
		{
			if (!ie) { yearNow += 1900	}<!--el 00 me coloca los 2 ultimos digitos del año,el 19 coje el año 1 y le suma 19 con esto nos muestra 2012-->
...
//CAMBIA el siguiente for por este " for	(i=yearNow; i<=(yearNow+4); i++) { " si quieres que los años empizen en el actual en adelante	con el mas le coloco los anos por encima y por debajo que quiero que se muestre en el calendario	
			for	(i=yearNow+2; i>=(yearNow-6); i--) {		
				sName =	i;
				if (i==yearSelected){
					sName =	"<B>" +	sName +	"</B>"
				}
